...
pub const Request = struct {
    ...
    pub fn header(self: Request, key: [:0]const u8) ?[:0]const u8 {
        const ptr = c.request_header(self.handle, key.ptr) orelse return null;
        return std.mem.span(ptr);
    }
...
pub const Response = struct {
    ...
    pub fn set_header(self: Response, key: [:0]const u8, value: [:0]const u8) void {
        c.response_set_header(self.handle, key.ptr, value.ptr);
    }
...
test "read request header" {
    var server = Server.init();
    defer server.deinit();

    _ = server.Get("/header", struct {
        fn handle(req: Request, res: Response) void {
            res.set_content(req.header("HX-Request") orelse "none", "text/plain");
        }
    }.handle);
    ...
    const url = try urlFor(std.testing.allocator, port, "/header");
    defer std.testing.allocator.free(url);
    ...
    try std.testing.expectEqual(std.http.Status.ok, res.status);
    try std.testing.expectEqualStrings("true", res.body);
}

test "set_status cookie and header" {
    var server = Server.init();
    defer server.deinit();
    ...
    while (it.next()) |header| {
        ...
        if (std.ascii.eqlIgnoreCase(header.name, "Vary") and
            std.mem.eql(u8, header.value, "HX-Request"))
        {
            found_vary = true;
        }
    }
    try std.testing.expect(found_cookie);
    try std.testing.expect(found_vary);
}
