...
fn RunXa6(allocator: std.mem.Allocator, body: [:0]const u8, args: []const [:0]const u8) [:0]const u8 {
    const mapped = allocator.alloc([:0]const u8, args.len) catch return "";
    defer allocator.free(mapped);
    var built: usize = 0;
    defer for (mapped[0..built]) |m| allocator.free(m);
    for (args, 0..) |arg, i| {
        mapped[i] = std.fmt.allocPrintSentinel(allocator, "--{s}", .{arg}, 0) catch return "";
        built += 1;
    }

    const c_args = allocator.alloc([*c]const u8, mapped.len) catch return "";
    defer allocator.free(c_args);
    for (mapped, 0..) |m, i| c_args[i] = m.ptr;
