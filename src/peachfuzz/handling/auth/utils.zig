...
pub fn provisionUser(db: *sqlite3.Sqlite3, username: [:0]const u8) !void {
    var stmt = try db.stmt("INSERT INTO auth_user (username, role) VALUES (?, ?)");
    defer stmt.deinit();
    try stmt.bindText(1, username);
    try stmt.bindInt64(2, @intFromEnum(accessly.Role.analyst));
    _ = try stmt.step();
}
