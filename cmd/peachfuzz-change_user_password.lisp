...
;;;; DEPRECATED (2026-08-01): peachfuzz no longer stores passwords.
;;;; The `auth_user.password` column was dropped from db/migrations and the
;;;; matching `src/peachfuzz/handling/auth/securing.zig` was deleted when local
;;;; sign-in moved to the urmom gRPC `Authenticate` call. The `UPDATE auth_user
;;;; SET password = ?` below now targets a column that no longer exists and will
;;;; fail at runtime. Change passwords in urmom instead. Kept for reference only.
...
(defun usage ()
  (opts:describe :prefix "[DEPRECATED] Change a peachfuzz user's password — the auth_user.password column was removed (passwords now live in urmom), so this tool no longer works."
