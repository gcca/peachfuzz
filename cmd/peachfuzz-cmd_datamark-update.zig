...
pub fn main() !void {
    ...
    if (gh_sources.len == 0 and s3_sources.len == 0 and drive_names.len == 0) {
        std.debug.print("no sources in datamark_source_update\n", .{});
        return;
    }
    ...
    for (gh_sources) |source| {
        updateSource(allocator, conn, source) catch |err| {
...
fn loadGithubSources(allocator: std.mem.Allocator, db: *c.sqlite3) ![]SourceInfo {
    const sql =
        ...
        \\FROM datamark_source_update up
        \\JOIN datamark_source ds ON ds.name = up.source_name
...
fn loadS3Sources(allocator: std.mem.Allocator, db: *c.sqlite3) ![]SourceInfo {
    const sql =
        \\SELECT ds.name, s3.bucket, s3.medallion, s3.pattern
        \\FROM datamark_source_update up
        \\JOIN datamark_source ds ON ds.name = up.source_name
        \\JOIN datamark_source_s3 s3 ON s3.source_name = ds.name
        \\ORDER BY ds.name, s3.medallion
    ;
