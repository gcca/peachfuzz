...
const db_path = "data/peachfuzz.db";
...
pub fn main() !void {
    ...
    const gh_token = std.mem.span(std.c.getenv("GH_TOKEN") orelse {
...
fn loadGithubSources(allocator: std.mem.Allocator, db: *c.sqlite3) ![]GithubSource {
    ...
    var list: std.ArrayList(GithubSource) = .empty;
    errdefer {
...
fn loadDriveSources(allocator: std.mem.Allocator, db: *c.sqlite3) ![]DriveSource {
    ...
    var list: std.ArrayList(DriveSource) = .empty;
    errdefer {
        for (list.items) |s| s.deinit(allocator);
        list.deinit(allocator);
    }
...
fn cloneGithubSource(
    ...
    cwd.createDirPath(io, out_dir) catch |err| {
    ...
    try downloadGithubAsset(allocator, io, &client, gh_token, asset_url, out_dir, source.asset);
